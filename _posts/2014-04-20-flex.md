...
```
##控制应用程序的外观
1. 大小/Sizes，即组件或应用程序的高度和宽度。2. 样式/Styles，即一组特性，如字体、排列方式、颜
...
2. 事件模型标准。Flex 事件模型是文档对象模型/Document Object Model （DOM）第三级事件的
一个子集，该模型是由World Wide Web Consortium（W3C）起草制定。
